...
async function source([collection,path]){if(window.MSGRepo&&window.MSGRepo.list&&window.MSGRepo.text){const paths=await window.MSGRepo.list(path,'.md');return (await Promise.all(paths.map(async file=>{try{return parse(await window.MSGRepo.text(file),file.split('/').pop(),collection)}catch(_){return null}}))).filter(Boolean)}const r=await fetch(`${REPO}${path}?ref=main&_=${Date.now()}`,{cache:'no-store'});if(r.status===404)return [];if(!r.ok)throw new Error(`${path}: ${r.status}`);const data=await r.json();if(!Array.isArray(data))return [];const fs=data.filter(x=>x.type==='file'&&/\.md$/i.test(x.name));return (await Promise.all(fs.map(async f=>{const q=await fetch(`${f.download_url}${f.download_url.includes('?')?'&':'?'}_=${Date.now()}`,{cache:'no-store'});return q.ok?parse(await q.text(),f.name,collection):null}))).filter(Boolean)}
async function load(){try{subcategories=await window.MSGSubcategories.ready;const settled=await Promise.allSettled(SOURCES.map(source));const ok=settled.filter(x=>x.status==='fulfilled');if(!ok.length)throw new Error('all news sources failed');settled.forEach((x,i)=>{if(x.status==='rejected')console.warn('[whatsnew] source skipped',SOURCES[i][1],x.reason)});const all=settled.flatMap(x=>x.status==='fulfilled'?x.value:[]),now=new Date();let items=all.filter(a=>visible(a,now)).sort((a,b)=>stamp(b)-stamp(a)).slice(0,15);items=await Promise.all(items.map(youtubeMeta));const top=items.slice(0,5),rest=items.slice(5,15);host.innerHTML=top.length?`<div class="wn-main-list">${top.map(mainCard).join('')}</div>${rest.length?`<div class="wn-older"><div class="wn-older-head"><span>MORE UPDATES</span><strong>過去の新着</strong></div><div class="wn-compact-list">${rest.map(compact).join('')}</div></div>`:''}`:'<p class="cms-events-status">現在、掲載中の新着情報はありません。</p>'}catch(err){console.error('[whatsnew]',err);host.innerHTML='<p class="cms-events-status">新着情報を読み込めませんでした。</p>'}}
load();})();
