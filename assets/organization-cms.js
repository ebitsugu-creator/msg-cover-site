...
function card(a){const url=`article.html?collection=${encodeURIComponent(a.collection)}&file=${encodeURIComponent(a.fileName)}`,img=image(a.image1||a.image2||''),qa=a.category==='qa'||a.articleType==='qa';return `<article class="cms-org-card">${img?`<a class="cms-org-card-image" href="${url}"><img src="${esc(img)}" alt="" loading="lazy"></a>`:''}<div class="cms-org-card-body"><div class="cms-org-card-meta">${qa?'<span class="cms-org-kind">Q&amp;A</span>':'<span class="cms-org-kind">記事</span>'}${a.publishedAt?`<time datetime="${esc(a.publishedAt)}">${esc(date(a.publishedAt))}</time>`:''}</div><h3><a href="${url}">${esc(a.title||'無題')}</a></h3>${a.subtitle?`<p class="cms-org-card-subtitle">${esc(a.subtitle)}</p>`:''}${a.summary?`<p class="cms-org-card-summary">${esc(a.summary)}</p>`:''}</div></article>`}
let all=[],group='',sub='';
function render(){
