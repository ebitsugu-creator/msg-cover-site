...
function thumb(a){const manual=image(a.image1||a.image2||''),id=yt(a.videoUrl||'');if(a.collection==='videos'&&id)return `https://i.ytimg.com/vi/${id}/hqdefault.jpg`;return manual||(id?`https://i.ytimg.com/vi/${id}/hqdefault.jpg`:'')}
...
function visible(a,now){if(a.publishable!==true||a.whatsNew!==true||a.isDraft===true)return false;const s=dt(a.publishStartAt),e=dt(a.publishEndAt);if(s&&now<s)return false;if(e&&now>e)return false;if(a.subcategory==='events_advance'){const ev=dt(a.eventAt);if(ev&&now>=ev)return false}const st=stamp(a);if(!st)return false;const limit=new Date(now);limit.setMonth(limit.getMonth()-3);return st>=limit}
...
function meta(a){return `<div class="wn-meta"><time datetime="${esc(a.publishStartAt||a.publishedAt||'')}">${esc(fmt(stamp(a)))}</time><span class="wn-subcat">${esc(LABELS[a.subcategory]||a.subcategory||'お知らせ')}</span>${a.subcategory==='events_advance'&&a.eventAt?`<span class="wn-event">開催 ${esc(fmt(a.eventAt,true))}</span>`:''}</div>`}
function mainCard(a){
 const h=href(a),im=thumb(a),attrs=h.ext?' target="_blank" rel="noopener noreferrer"':'';
 ...
 return `<article class="wn-main-card${im?'':' wn-main-card--noimage'}${a.summary?'':' wn-main-card--nosummary'}">${im?`<a class="wn-main-image" href="${esc(h.url)}"${attrs}><img src="${esc(im)}" alt="" loading="lazy"></a>`:''}<div class="wn-main-body"><div class="wn-topline">${date}${sub}<h2><a href="${esc(h.url)}"${attrs}>${esc(a.title||'無題')}</a></h2>${event}</div>${a.summary?`<p class="wn-main-summary">${esc(a.summary)}</p>`:''}</div></article>`
}
function compact(a){
 const h=href(a),attrs=h.ext?' target="_blank" rel="noopener noreferrer"':'';
 const sub=esc(LABELS[a.subcategory]||a.subcategory||'お知らせ');
 return `<article class="wn-compact-line"><time datetime="${esc(a.publishStartAt||a.publishedAt||'')}">${esc(fmt(stamp(a)))}</time><span class="wn-compact-subcat">${sub}</span><h3><a href="${esc(h.url)}"${attrs}>${esc(a.title||'無題')}</a></h3><p>${a.summary?esc(a.summary):''}</p></article>`
}
