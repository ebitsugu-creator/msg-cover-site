...
function mainCard(a){
 ...
 return `<article class="wn-main-card${im?'':' wn-main-card--noimage'}${a.summary?'':' wn-main-card--nosummary'}">${im?`<a class="wn-main-image" href="${esc(h.url)}"${attrs}><img src="${esc(im)}" alt="" loading="lazy"></a>`:''}<div class="wn-main-body"><div class="wn-topline">${date}${sub}<h2><a href="${esc(h.url)}"${attrs}>${esc(displayTitle(a))}</a></h2>${event}</div>${a.summary?`<p class="wn-main-summary">${esc(a.summary)}</p>`:''}</div></article>`
}
function compact(a){
 const h=href(a),attrs=h.ext?' target="_blank" rel="noopener noreferrer"':'';
 const sub=esc(LABELS[a.subcategory]||a.subcategory||'お知らせ');
 return `<article class="wn-compact-line"><time datetime="${esc(a.publishStartAt||a.publishedAt||'')}">${esc(fmt(stamp(a)))}</time><span class="wn-compact-subcat">${sub}</span><h3><a href="${esc(h.url)}"${attrs}>${esc(displayTitle(a))}</a></h3><p>${a.summary?esc(a.summary):''}</p></article>`
}
